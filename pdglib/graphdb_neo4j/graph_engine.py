...
def expand_subgraph( graphdb, gid, uuids, limit=10):
    ...
    for u in uuids:
        p = graphdb.proxemie( gid, [u], weights, filter_edges=filter_edges, filter_nodes=filter_nodes, limit=limit, n_step=step)
        
        vs = vs + p.keys()

    return nodes_subgraph(graphdb, gid, list(set(vs)) )

    
    
def prox_subgraph( graphdb, gid, p0_uuids, size=50):

    size = min(size, 200 )

    prox = graphdb.proxemie(gid, p0_uuids, limit=size )
    uuids = prox.keys()
