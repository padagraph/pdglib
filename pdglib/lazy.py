...
from graphdb_interface import GraphExistsError, GraphNameError, NodeNotFoundError, EdgeNotFoundError, UserLoginNotFoundError
...
class iGraphDB(graphdb_interface.iGraphDB):
    ...
    def open_database(self):
        for k,v in self.conf.items():
            print "opening graph %s@%s" %(k, v)
            self.graphs[k] = IgraphGraph.Read(v)
    ...
    def get_graph(self, gid):
        ...
        if not g : raise GraphNameError('%s' % gid)
        return g
